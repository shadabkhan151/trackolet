...
import com.trackolet.trackolet.helper.DataConverter;
...
import com.trackolet.trackolet.model.CellTower;
import com.trackolet.trackolet.model.Network;
...
public class StarLinkProtocolDecoder extends BaseProtocolDecoder {
    ...
    protected void init() {
        setFormat(getConfig().getString(
                Keys.PROTOCOL_FORMAT.withPrefix(getProtocolName()).getKey(), "#EDT#,#EID#,#PDT#,#LAT#,#LONG#,#SPD#,#HEAD#,#ODO#,"
                + "#IN1#,#IN2#,#IN3#,#IN4#,#OUT1#,#OUT2#,#OUT3#,#OUT4#,#LAC#,#CID#,#VIN#,#VBAT#,#DEST#,#IGN#,#ENG#"));

        setDateFormat(getConfig().getString(Keys.PROTOCOL_DATE_FORMAT.withPrefix(getProtocolName()).getKey(), "yyMMddHHmmss"));
    }
